...
HostAI has a separate guest chat page with expiring, revocable, model-specific keys. Optional Cloudflare Quick Tunnel sharing adds temporary internet access after a secure WebSocket reachability check; local keys and internet keys have separate permissions. Cloudflare can see relayed messages and access keys. An optional self-hostable directory adds host search and explicit listing publication; no public registry is deployed or configured by default. **Stable public hosting, verified host identities, persistent conversations, signed installers, and bundled Java runtimes are not implemented.** See [host directory](docs/directory.md). Hosts can separately enable [guest access requests](docs/access-requests.md), choose an expiry, and approve or reject each guest in the app. Owner controls stay on loopback. See [guest access](docs/guest-access.md) for setup, limits and the current verification boundary. Runtime and request metrics come from the backend, never sample data.
